...
from service.user_base_service import UserBaseService
from service.base_service import BaseService
from service.constant_service import CONSTANT_SERVICE
from service.log_service import auto_log
from service.workflow_base_service import WorkflowBaseService
from service.workflow_custom_field_service import WorkflowCustomFieldService
from service.workflow_state_service import WorkflowStateService
from service.workflow_transition_service import WorkflowTransitionService
...
class TicketBaseService(BaseService):
    ...
    @classmethod
    @auto_log
    def get_ticket_list(cls, sn='', title='', username='', create_start='', create_end='', category='', reverse=1, per_page=15, page=1):
        """
        工单列表
        :param sn:
        :param title:
        ...
        if category == 'owner':
            ...
            # 获取用户部门id列表, 角色id列表，工单的实际当前处理人只会有个人。
            user_obj, msg = UserBaseService.get_user_by_username(username)
            if not user_obj:
                return False, msg
            user_dept_id_list, msg2 = UserBaseService.get_user_up_dept_id_list(username)
            user_role_id_list, msg3 = UserBaseService.get_user_role_id_list(username)
